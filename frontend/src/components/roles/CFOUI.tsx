...
import { CheckCircle2, XCircle, Receipt, User, Clock, ShieldCheck } from 'lucide-react';
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 gap-4">
        {pending.length === 0 ? (
          ...
          pending.map(exp => (
            <Card key={exp.id} className="flex items-center justify-between border-emerald-100 bg-emerald-50/10">
              ...
              <div className="flex items-center gap-8">
                <div className="text-right">
                  <p className="text-2xl font-bold text-emerald-700">{exp.currency} {exp.amount}</p>
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => handleAction(exp.id, 'Approve')} className="p-3 rounded-xl bg-emerald-600 text-white hover:bg-emerald-700 transition-colors shadow-lg shadow-emerald-200"><CheckCircle2 size={20} /></button>
                </div>
              </div>
            </Card>
          ))
